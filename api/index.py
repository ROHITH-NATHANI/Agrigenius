...
# Simple in-memory LRU cache (best-effort; may not persist on serverless)
...
def _prompt_from_history(system_instruction: str, history: list, message: str) -> str:
    ...
    # History items are expected like: { role: 'user'|'assistant'|'system', content: '...' }
...
def _extract_json_object(text: str) -> dict | None:
    ...
    # Best-effort: grab the first JSON object in the string
    start = cleaned.find("{")
    end = cleaned.rfind("}")
...
def gemini_endpoint():
    ...
    try:
        ...
        # Fallback if model didn't follow the strict JSON requirement
